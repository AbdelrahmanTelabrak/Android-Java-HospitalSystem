...
import com.example.hospitalsystem_abdelrahmantarek.databinding.FragmentNurseCasesListBinding;

import java.util.ArrayList;

public class NurseCasesListFragment extends Fragment {
    FragmentNurseCasesListBinding binding;
    NavController navController;
    CasesListViewModel casesListViewModel;
    CaseDetailsViewModel caseDetailsViewModel;
    DocCasesListAdaptor adaptor;

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        // Inflate the layout for this fragment
        binding = DataBindingUtil.inflate(inflater, R.layout.fragment_nurse_cases_list, container, false);
    ...
    public void casesListObserver(){
        casesListViewModel.getCasesListLiveData().observe(getViewLifecycleOwner(), new Observer<ArrayList<CaseCardData>>() {
            ...
            public void onChanged(ArrayList<CaseCardData> caseCardData) {
                adaptor = new DocCasesListAdaptor(caseCardData, binding.getRoot().getContext(), caseDetailsViewModel);
                binding.rvNCLCases.setAdapter(adaptor);
            }
        });
